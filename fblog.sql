...
create database if not exists fBlog default character set utf8;

Date: 2017-01-10 21:51:26
*/
...
-- Records of link
-- ----------------------------
INSERT INTO `link` VALUES ('6ChkuVe0kfLY8ZiDnqZ', 'Hello Fansion', 'http://weibo.com/u/2164052835/home?wvr=5', null, '1', 'admin', '2017-01-12 17:27:31');
...
-- Records of options
-- ----------------------------
INSERT INTO `options` VALUES ('title','title','Fblog');
INSERT INTO `options` VALUES ('subtitle','subtitle','Just a new fBlog website');
INSERT INTO `options` VALUES ('description','description','start learn spring and spring mvc,write a blog for start');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_category`
AS select `n`.`id` AS `id`,`n`.`name` AS `name`,count(`n`.`id`) AS `level`,`n`.`visible` AS `visible` from (`category` `n` join `category` `p`)
where (`n`.`leftv` between `p`.`leftv` and `p`.`rightv`) group by `n`.`id` order by `n`.`leftv` ;
